...
def main():
    ...
    ms = MSSQL(host="106.14.41.25:3539", user="saw", pwd="ncist1525", db="DTU_SERVER_NEW")  # 实例化类对象，连接数据对象
    timestr='服务器时间'
    str0='B相电流'        #'B相电流'
    str1='B相电场'          #'B相电场'
    DTU_add='65001'
    ...
    pl.figure(num=4, figsize=(40, 30))
    pl.subplot(211)
    pl.plot(t, x0, label=str0)
    pl.legend(prop=myfont)
    # pl.xticks(rotation=45)  # 设置时间标签显示格式
    pl.title(x_time[-1] + ' 到 ' + x_time[0] + '  图形=' + DTU_add + ',数量=' + str(len(x0)) + '/' + count_num,
